...
* A partner account belongs to you, i.e. a partner that is tech-wise integrated
  into the transaction gateway.
...
easy rolling of API keys. A merchant account usually has one API key.

API keys can have an expire date but usually do not. API keys can be enabled or
disabled. For merchant accounts, these details can be observed in the Clearhaus
Dashboard.

An API key can be a signing API key; a signing API key has a verification key
that can verify signatures. This verification key is the public part of an RSA
key pair; the RSA key pair is generated by the partner. The public part, the
verification key, is communicated to Clearhaus during the technical integration.

Only partner accounts have signing API keys (because Clearhaus puts trust in
partners to process transactions correctly). Thus, in the `Signature` header,
the partner's signing API key is supplied together with the hex signature; this
hex signature is created using the private part of the RSA key pair such that
the hex signature can be verified using the verification key that belongs to
that signing API key.
